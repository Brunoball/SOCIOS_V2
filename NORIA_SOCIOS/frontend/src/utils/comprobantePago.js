...
export const normalizePaymentReceipt = (source = {}) => {
  ...
  const lines = (operation.lineas || safeSource.lineas || []).map((line, index) => ({
    ...
    montoSaldoFavorAplicado: Number(line.monto_saldo_favor_aplicado ?? 0),
  }));
  ...
  return {
    ...
    montoSaldoFavorAplicado: Number(
      operation.monto_saldo_favor_aplicado ??
        lines.reduce(
          (total, line) => total + line.montoSaldoFavorAplicado,
          0,
        ),
    ),
    montoCobradoAhora: Number(
      operation.monto_cobrado_ahora ??
        operation.monto ??
        lines.reduce((total, line) => total + line.monto, 0),
    ),
    sobranteSaldoFavor: Number(operation.sobrante_saldo_favor || 0),
  ...
  <html lang="es">
    ...
    <body>
      ...
      <main class="receipt">
        ...
        ${receipt.montoSaldoFavorAplicado > 0 || receipt.sobranteSaldoFavor > 0 ? `<p class="note"><strong>Detalle del cobro:</strong> ${receipt.montoSaldoFavorAplicado > 0 ? `Monto a favor aplicado ${htmlEscape(money(receipt.montoSaldoFavorAplicado))} · ` : ""}Recibido ahora ${htmlEscape(money(receipt.montoCobradoAhora))}${receipt.sobranteSaldoFavor > 0 ? ` · Sobrante guardado a favor ${htmlEscape(money(receipt.sobranteSaldoFavor))}` : ""}</p>` : ""}
        <table>
...
const paymentReceiptPdfContent = (receipt, pageLines, page, totalPages) => {
  ...
  if (page === totalPages) {
    ...
    if (receipt.montoSaldoFavorAplicado > 0) {
      commands.push(
        pdfTextCommand(40, totalY + 8, 7.5, `Monto a favor aplicado: ${money(receipt.montoSaldoFavorAplicado)}`, {
          bold: true,
          color: muted,
        }),
      );
      commands.push(
        pdfTextCommand(40, totalY - 4, 7.5, `Recibido ahora: ${money(receipt.montoCobradoAhora)}`, { color: muted }),
      );
    }
    if (receipt.sobranteSaldoFavor > 0) {
      commands.push(
        pdfTextCommand(40, totalY - 16, 7.5, `Sobrante a favor: ${money(receipt.sobranteSaldoFavor)}`, {
          bold: true,
          color: accent,
        }),
      );
