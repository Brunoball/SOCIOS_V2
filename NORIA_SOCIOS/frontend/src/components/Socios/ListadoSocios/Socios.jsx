...
export default function Socios() {
  ...
  const deletePermanently = async ({ motivo }) => {
    if (!deleteModal) return null;
    const response = await sociosApi.eliminarDefinitivo(
      deleteModal.id_socio,
      motivo,
    );
    await cargar();
    return response;
    ...
    <>
      ...
      >
        ...
        <GlobalDivTable
          ...
          skeletonRows={8}
          skeletonActionCount={writable ? 4 : 1}
          skeletonColumnTypes={[
            "stacked",
            "line",
            "line",
      ...
      <ModalEliminarGlobal
        ...
        message="El socio desaparecerá de la operatoria diaria, pero sus pagos, inscripciones, ventas, saldos y auditoría se conservarán para trazabilidad."
        warning="Esta acción no se puede revertir desde el sistema. El DNI quedará liberado para una nueva alta futura."
        details={
          deleteModal
            ? [
                {
                  label: "Socio",
                  value: `${deleteModal.apellido}, ${deleteModal.nombre}`,
                },
                { label: "DNI", value: deleteModal.dni },
                {
                  label: "Estado actual",
                  value: deleteModal.activo ? "ACTIVO" : "BAJA",
                },
              ]
            : []
        }
        ...
        loadingMessage="Archivando al socio y preservando su historial…"
        successMessage="Socio eliminado de la operatoria. El historial quedó conservado."
        errorMessage="No se pudo eliminar definitivamente al socio."
        showReason
        reasonRequired
        reasonLabel="Motivo de eliminación *"
        reasonPlaceholder="Indicá por qué se elimina definitivamente..."
