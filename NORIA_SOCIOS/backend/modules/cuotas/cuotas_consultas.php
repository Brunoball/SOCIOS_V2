...
abstract class CuotasConsultas extends CuotasSoporte
{
    protected static function listarDatos(PDO $db, array $filters): array
    {
        ...
        if (!in_array($tab, ['deudores', 'pagados', 'condonados', 'saldos'], true)) {
        ...
        $result = match ($tab) {
            'deudores' => self::listarDeudores($db, $search, $categoryId, $year, $month),
            'saldos' => self::listarSaldosFavor($db, $search),
            default => self::listarOperaciones(
                $db,
    ...
    protected static function catalogosDatos(PDO $db): array
    {
        $socios = $db->query(
            'SELECT s.id_socio, s.apellido, s.nombre, s.dni, f.nombre AS familia
             FROM socios s
             LEFT JOIN familia_socios fs ON fs.id_socio = s.id_socio AND fs.fecha_desvinculacion IS NULL
    ...
    protected static function listarSaldosFavor(PDO $db, string $search): array
    {
        $where = ['NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)'];
        $params = [];
        if ($search !== '') {
            $term = '%' . $search . '%';
            $where[] = '(s.apellido LIKE ? OR s.nombre LIKE ? OR s.dni LIKE ?)';
        ...
        $statement = $db->prepare(
            "SELECT s.id_socio,
                    ...
                    s.dni,
                    s.activo,
                    COALESCE(SUM(sf.monto), 0) AS saldo_favor,
                    COUNT(sf.id_movimiento) AS cantidad_movimientos,
                    MAX(sf.fecha) AS fecha_ultimo_movimiento
             FROM socios s
             LEFT JOIN saldos_favor_movimientos sf ON sf.id_socio = s.id_socio
             {$sqlWhere}
             GROUP BY s.id_socio, s.apellido, s.nombre, s.dni, s.activo
             HAVING COALESCE(SUM(sf.monto), 0) > 0.004
             ORDER BY COALESCE(SUM(sf.monto), 0) DESC,
                      s.apellido, s.nombre"
        );
        ...
        foreach ($items as &$item) {
            ...
            $item['cantidad_movimientos'] = (int)$item['cantidad_movimientos'];
            $balance = round((float)$item['saldo_favor'], 2);
            $item['saldo_favor'] = number_format($balance, 2, '.', '');
            $total += $balance;
            if ($balance > 0.004) $withBalance++;
        }
        unset($item);
    ...
    ): array
    {
        ...
        foreach ($itemsByPartnerCategory as $item) {
            ...
            if ($periods === []) continue;
            $item['periodos_pendientes'] = $periods;
    ...
    {
        $statement = $db->prepare('SELECT s.id_socio, s.apellido, s.nombre, s.dni, s.fecha_ingreso, s.activo FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)');
        ...
        if ((bool)$principal['activo']) {
            $familyStatement = $db->prepare(
                'SELECT f.id_familia, f.nombre
                 ...
                 WHERE fs.id_socio = ? AND fs.fecha_desvinculacion IS NULL AND f.activo = 1 LIMIT 1'
            );
        ...
        if ($family) {
            $membersStatement = $db->prepare(
                'SELECT s.id_socio, s.apellido, s.nombre, s.dni, s.fecha_ingreso, s.activo
                 ...
                 WHERE fs.id_familia = ? AND fs.fecha_desvinculacion IS NULL AND s.activo = 1
                   AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)
                 ORDER BY s.apellido, s.nombre'
            );
        ...
        return [
            ...
            'saldo_favor' => number_format(self::saldoFavorActual($db, $partnerId), 2, '.', ''),
            'medios_pago' => self::mediosPagoCatalogo($db),
            'modalidades' => self::modalidadesCatalogo($db),
    ...
    protected static function paymentRows(PDO $db, ?string $status = null, ?string $code = null, ?int $id = null): array
    {
        ...
        $observationsExpression = $has('observaciones')
            ? 'p.observaciones'
            : 'NULL';
        $balanceAppliedExpression = $has('monto_saldo_favor_aplicado')
        ...
        $statement = $db->prepare(
            "SELECT 'CUOTA' AS tipo_registro,
                    ...
                    p.monto,
                    {$balanceAppliedExpression} AS monto_saldo_favor_aplicado,
                    p.fecha_pago,
                    p.estado,
    ...
    protected static function groupOperations(array $rows, bool $separateByPartner = false): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($grouped[$groupKey])) {
                $grouped[$groupKey] = [
                    ...
                    'monto_base_num' => 0.0,
                    'monto_num' => 0.0,
                    'saldo_aplicado_num' => 0.0,
                    'es_monto_libre' => false,
                    'lineas' => [],
                ];
            }
            ...
            $grouped[$groupKey]['saldo_aplicado_num'] += (float)($row['monto_saldo_favor_aplicado'] ?? 0);
            $expectedLineAmount = round(
                (float)$row['monto_base']
                * (1 - ((float)($row['porcentaje_descuento_modalidad'] ?? 0) / 100))
                * (1 - ((float)($row['porcentaje_descuento_familiar'] ?? 0) / 100)),
                2
            );
            $lineIsFreeAmount = ($row['estado'] ?? '') === 'PAGADO'
                && abs((float)$row['monto'] - $expectedLineAmount) > 0.009;
            if ($lineIsFreeAmount) {
                $grouped[$groupKey]['es_monto_libre'] = true;
            }
            ...
            $grouped[$groupKey]['lineas'][] = [
                ...
                'monto_saldo_favor_aplicado' => number_format((float)($row['monto_saldo_favor_aplicado'] ?? 0), 2, '.', ''),
                'monto_cobrado_ahora' => number_format(max(0, (float)$row['monto'] - (float)($row['monto_saldo_favor_aplicado'] ?? 0)), 2, '.', ''),
                'es_monto_libre' => $lineIsFreeAmount,
            ];
        }
        ...
        foreach ($grouped as $operation) {
            ...
            $isMulti = preg_match('/^MULTI-CUO-/', (string)$operation['codigo_operacion']) === 1;
            $modalityCode = $isCombined
                ? 'COBRO_COMBINADO'
                : ($isMulti ? 'PAGO_MULTIPLE' : (count($modalityCodes) === 1 ? $modalityCodes[0] : 'VARIAS'));
            $modalityLabel = $isCombined
                ? 'CUOTAS + INSCRIPCIÓN'
                : ($isMulti ? 'PAGO MÚLTIPLE' : (count($modalityNames) === 1 ? $modalityNames[0] : 'VARIAS MODALIDADES'));
            ...
            $result[] = [
                ...
                'monto_saldo_favor_aplicado' => number_format($operation['saldo_aplicado_num'], 2, '.', ''),
                'monto_cobrado_ahora' => number_format(max(0, $operation['monto_num'] - $operation['saldo_aplicado_num']), 2, '.', ''),
                'es_monto_libre' => (bool)$operation['es_monto_libre'],
    ...
    protected static function operacionPorCodigo(PDO $db, string $code): ?array
    {
        ...
        $surplus = $db->prepare(
            "SELECT COALESCE(SUM(monto), 0)
             ...
             WHERE referencia_externa = ? AND tipo = 'SOBRANTE' AND monto > 0"
        );
        $surplus->execute([$operation['codigo_operacion']]);
        $surplusAmount = round((float)$surplus->fetchColumn(), 2);
        $operation['sobrante_saldo_favor'] = number_format($surplusAmount, 2, '.', '');
        $operation['monto_cobrado_ahora'] = number_format(
            max(0, (float)$operation['monto'] - (float)$operation['monto_saldo_favor_aplicado']) + $surplusAmount,
            2,
            '.',
            ''
        );
