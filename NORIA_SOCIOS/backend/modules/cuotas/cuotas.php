...
final class Cuotas extends CuotasRegistros
{
    ...
    public static function ajustarSaldo(): never
    {
        $auth = require_admin();
        $result = self::ajustarSaldoDatos($auth, request_body());
        api_success($result, 'Monto a favor actualizado correctamente.');
    }

    public static function registrarPagoMultiple(): never
    {
        $auth = require_admin();
        $result = self::registrarPagoMultipleDatos($auth, request_body());
        api_success($result, 'Pago múltiple registrado correctamente.');
    }
