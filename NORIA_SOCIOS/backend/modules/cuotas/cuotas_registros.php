...
abstract class CuotasRegistros extends CuotasConsultas
{
    ...
    protected static function registrarInscripcionDatos(array $auth, array $body, ?array $context = null): array
    {
        ...
        try {
            ...
            $charged = 0.0;
            $theoretical = 0.0;
            $partnerLock = $db->prepare('SELECT s.id_socio FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio) FOR UPDATE');
            foreach ($recipients as $partnerId) {
                ...
                if ($reusable) {
                    $update = $db->prepare(
                        'UPDATE pagos_inscripciones SET codigo_operacion = ?, id_familia = ?, id_medio_pago = ?, descripcion = ?, anio = ?,
                         monto_base = ?, porcentaje_descuento_familiar = ?, monto = ?, monto_saldo_favor_aplicado = 0, fecha_pago = ?, estado = ?,
    ...
    protected static function ajustarSaldoDatos(array $auth, array $body): array
    {
        $db = $auth['db'];
        $partnerId = positive_id($body['id_socio'] ?? null, 'socio');
        $kind = self::upper(clean_text($body['tipo'] ?? '', 20, false));
        if (!in_array($kind, ['CREDITO', 'DEBITO', 'FIJAR', 'ELIMINAR'], true)) {
            api_error('El tipo de ajuste de saldo no es válido.', 'VALIDATION_ERROR');
        }
        ...
        if ($kind !== 'ELIMINAR') {
            ...
            if ($rawAmount === '' || !is_numeric($rawAmount)) {
                api_error('Indicá un monto válido para el ajuste.', 'VALIDATION_ERROR');
            }
            $amount = round((float)$rawAmount, 2);
            $minimum = $kind === 'FIJAR' ? 0 : 0.01;
            if ($amount < $minimum || $amount > 999999999.99) {
                api_error(
                    $kind === 'FIJAR'
                        ? 'El monto a favor no puede ser negativo.'
                        : 'El monto del ajuste debe ser mayor a cero.',
                    'VALIDATION_ERROR'
                );
            }
        }

        $defaultDetail = match ($kind) {
            'CREDITO' => 'Monto a favor agregado manualmente',
            'DEBITO' => 'Monto a favor descontado manualmente',
            'FIJAR' => 'Monto a favor editado manualmente',
            'ELIMINAR' => 'Monto a favor eliminado manualmente',
        };
        $detail = optional_text($body['detalle'] ?? null, 500) ?? $defaultDetail;

        return transaction($db, static function () use ($db, $auth, $partnerId, $kind, $amount, $detail): array {
            $before = self::saldoFavorActual($db, $partnerId, true);

            if ($kind === 'DEBITO' && (float)$amount > $before + 0.004) {
                api_error(
                    'El débito supera el monto a favor disponible del socio.',
                    'SALDO_FAVOR_INSUFICIENTE',
                    409
                );
            }

            if ($kind === 'ELIMINAR' && $before <= 0.004) {
                api_error('El socio no tiene monto a favor para eliminar.', 'SALDO_FAVOR_INEXISTENTE', 409);
            }

            $after = match ($kind) {
                'CREDITO' => round($before + (float)$amount, 2),
                'DEBITO' => round($before - (float)$amount, 2),
                'FIJAR' => round((float)$amount, 2),
                'ELIMINAR' => 0.0,
            };
            $signedAmount = round($after - $before, 2);
            ...
            if (abs($signedAmount) < 0.005) {
                return [
                    ...
                    'tipo' => $kind,
                    'monto' => '0.00',
                    'saldo_anterior' => number_format($before, 2, '.', ''),
                    'saldo_favor' => number_format($after, 2, '.', ''),
                ];
            }

            $movementType = $signedAmount > 0 ? 'AJUSTE_CREDITO' : 'AJUSTE_DEBITO';
            $reference = self::operationCode('SALDO-AJ');
            $movementId = self::registrarMovimientoSaldo(
                $db,
                $partnerId,
                $movementType,
                $signedAmount,
                ...
                $reference,
                'SALDO-AJUSTE-' . $reference,
                $detail
            );

            $auditAction = match ($kind) {
                'CREDITO' => 'AGREGAR_SALDO_FAVOR',
                'DEBITO' => 'DESCONTAR_SALDO_FAVOR',
                'FIJAR' => 'EDITAR_SALDO_FAVOR',
                'ELIMINAR' => 'ELIMINAR_SALDO_FAVOR',
            };
            $auditDescription = match ($kind) {
                'CREDITO' => 'Se agregó monto a favor manualmente.',
                'DEBITO' => 'Se descontó monto a favor manualmente.',
                'FIJAR' => 'Se editó el monto a favor manualmente.',
                'ELIMINAR' => 'Se eliminó el monto a favor disponible.',
            };
            ...
            audit_change(
                ...
                $movementId,
                $auditDescription,
                ['saldo_favor' => number_format($before, 2, '.', '')],
                [
                    'tipo' => $movementType,
                    'monto' => number_format($signedAmount, 2, '.', ''),
                    'saldo_favor' => number_format($after, 2, '.', ''),
                    'detalle' => $detail,
                ]
            );
    ...
    /**
     * Distribuye un monto libre entre las líneas de una operación manteniendo
     * exactitud a centavos. Las cuotas/inscripciones quedan registradas como
     * PAGADAS, pero su monto representa lo efectivamente aceptado.
     */
    protected static function aplicarMontoLibreOperacion(PDO $db, array $rows, float $freeAmount): void
    {
        if ($rows === []) {
            api_error('No hay líneas para aplicar el monto libre.', 'COBRO_SIN_LINEAS', 500);
        }

        $totalCents = (int)round($freeAmount * 100);
        if ($totalCents <= 0) {
            api_error('El monto libre debe ser mayor a cero.', 'MONTO_LIBRE_INVALIDO');
        }

        $weights = [];
        $weightTotal = 0;
        foreach ($rows as $row) {
            $weight = max(0, (int)round((float)($row['monto'] ?? 0) * 100));
            $weights[] = $weight;
            $weightTotal += $weight;
        }
        if ($weightTotal <= 0) {
            api_error('No se pudo calcular la distribución del monto libre.', 'MONTO_LIBRE_DISTRIBUCION', 500);
        }

        $remaining = $totalCents;
        $lastIndex = count($rows) - 1;
        foreach ($rows as $index => $row) {
            $lineCents = $index === $lastIndex
                ? $remaining
                : min($remaining, (int)floor(($totalCents * $weights[$index]) / $weightTotal));
            $remaining -= $lineCents;
            $lineAmount = number_format($lineCents / 100, 2, '.', '');

            if (($row['tipo_registro'] ?? '') === 'CUOTA') {
                $statement = $db->prepare(
                    'UPDATE pagos SET monto = ?, monto_saldo_favor_aplicado = 0 WHERE id_pago = ?'
                );
                $statement->execute([$lineAmount, (int)$row['id_linea']]);
            } elseif (($row['tipo_registro'] ?? '') === 'INSCRIPCION') {
                $statement = $db->prepare(
                    'UPDATE pagos_inscripciones SET monto = ?, monto_saldo_favor_aplicado = 0 WHERE id_pago_inscripcion = ?'
                );
                $statement->execute([$lineAmount, (int)$row['id_linea']]);
            } else {
                api_error('Una línea del monto libre no es válida.', 'MONTO_LIBRE_DISTRIBUCION', 500);
            }
        }
    }

    protected static function registrarPagoMultipleDatos(array $auth, array $body): array
    {
        $db = $auth['db'];
        $date = valid_date($body['fecha_pago'] ?? date('Y-m-d'), 'pago');
        if ($date > date('Y-m-d')) api_error('La fecha de pago no puede ser futura.', 'VALIDATION_ERROR');
        $mediumId = self::normalPaymentMediumId($db, $body['id_medio_pago'] ?? null);
        $requested = is_array($body['pagos'] ?? null) ? $body['pagos'] : [];
        if ($requested === [] || count($requested) > 500) {
            api_error('Seleccioná entre 1 y 500 cuotas para el pago múltiple.', 'VALIDATION_ERROR');
        }

        $normalized = [];
        foreach ($requested as $payment) {
            if (!is_array($payment)) api_error('Una cuota seleccionada no es válida.', 'VALIDATION_ERROR');
            $partnerId = positive_id($payment['id_socio'] ?? null, 'socio');
            $categoryId = positive_id($payment['id_categoria'] ?? null, 'categoría');
            $year = filter_var($payment['anio'] ?? null, FILTER_VALIDATE_INT, [
                'options' => ['min_range' => 2000, 'max_range' => self::maximumEnabledYear()],
            ]);
            $month = filter_var($payment['id_mes'] ?? null, FILTER_VALIDATE_INT, [
                'options' => ['min_range' => 1, 'max_range' => 12],
            ]);
            if ($year === false || $month === false) api_error('Uno de los períodos no es válido.', 'VALIDATION_ERROR');
            $key = self::periodKey($partnerId, $categoryId, (int)$year, (int)$month);
        ...
        if ($payments === []) api_error('No hay cuotas válidas seleccionadas.', 'VALIDATION_ERROR');

        usort($payments, static fn(array $a, array $b): int => [
            $a['id_socio'], $a['id_categoria'], $a['anio'], $a['id_mes'],
        ] <=> [
            $b['id_socio'], $b['id_categoria'], $b['anio'], $b['id_mes'],
        ]);

        $operationCode = self::operationCode('MULTI-CUO');
        $context = [
        ...
        $saved = transaction($db, static function () use (
            $db, $auth, $payments, $date, $mediumId, $operationCode, $context
        ): array {
            // Bloqueo estable de los socios y vínculos familiares involucrados.
            // El orden anterior evita deadlocks cuando dos cajas cobran lotes a la vez.
            $partnerIds = array_values(array_unique(array_column($payments, 'id_socio')));
            $placeholders = implode(',', array_fill(0, count($partnerIds), '?'));
            $partnerLock = $db->prepare(
                "SELECT s.id_socio
                 FROM socios s
                 WHERE s.id_socio IN ({$placeholders})
                   AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)
                 ORDER BY s.id_socio
                 FOR UPDATE"
            );
            $partnerLock->execute($partnerIds);
            $lockedPartnerIds = array_map('intval', $partnerLock->fetchAll(PDO::FETCH_COLUMN));
            if (count($lockedPartnerIds) !== count($partnerIds)) {
                api_error('Uno de los socios seleccionados ya no está disponible.', 'SOCIO_NO_DISPONIBLE', 409);
            }

            $familyLinkLock = $db->prepare(
                "SELECT id_familia_socio
                 FROM familia_socios
                 WHERE id_socio IN ({$placeholders}) AND fecha_desvinculacion IS NULL
                 ORDER BY id_familia_socio
                 FOR UPDATE"
            );
            $familyLinkLock->execute($partnerIds);
            $familyLinkLock->fetchAll(PDO::FETCH_COLUMN);

            $total = 0.0;
            $lineCount = 0;
            foreach ($payments as $payment) {
                $result = self::registrarPagoDatos($auth, [
                    'id_socio' => $payment['id_socio'],
                ...
                $total += (float)($result['monto'] ?? 0);
                $lineCount += (int)($result['lineas'] ?? 0);
            }

            $rows = self::paymentRows($db, null, $operationCode);
            $operation = self::groupOperations($rows)[0] ?? null;
            audit_change(
                $db,
                $auth,
                'CUOTAS',
                'REGISTRAR_PAGO_MULTIPLE',
                'pagos',
                $operationCode,
                'Se registró un pago múltiple de cuotas.',
                null,
                [
                    'codigo_operacion' => $operationCode,
                    'fecha_pago' => $date,
        ...
        return ['codigo_operacion' => $operationCode, 'estado' => 'PAGADO'] + $saved;
    }
    ...
    protected static function registrarCobroDatos(array $auth, array $body): array
    {
        ...
        $useFreeAmount = filter_var($body['usar_monto_libre'] ?? false, FILTER_VALIDATE_BOOL);
        $freeAmount = null;
        if ($useFreeAmount) {
            $freeRaw = trim((string)($body['monto_libre'] ?? ''));
            $normalizedFree = str_replace(',', '.', $freeRaw);
            if ($freeRaw === '' || !is_numeric($normalizedFree)) {
                api_error('Indicá un monto libre válido.', 'MONTO_LIBRE_INVALIDO');
            }
            $freeAmount = round((float)$normalizedFree, 2);
            if ($freeAmount <= 0 || $freeAmount > 999999999.99) {
                api_error('El monto libre debe ser mayor a cero.', 'MONTO_LIBRE_INVALIDO');
            }
        }
        $receivedRaw = trim((string)($body['monto_recibido'] ?? ''));
        $receivedAmount = null;
        if ($receivedRaw !== '') {
            $normalizedReceived = str_replace(',', '.', $receivedRaw);
            if (!is_numeric($normalizedReceived)) {
                api_error('El monto recibido no es válido.', 'VALIDATION_ERROR');
            }
            $receivedAmount = round((float)$normalizedReceived, 2);
            if ($receivedAmount < 0 || $receivedAmount > 999999999.99) {
                api_error('El monto recibido no es válido.', 'VALIDATION_ERROR');
            }
        }
        if ($useBalance && $applyFamily) {
            api_error(
                'El monto a favor es individual. Para utilizarlo, desactivá el cobro a todo el grupo familiar.',
                'SALDO_FAVOR_PAGO_FAMILIAR',
                409
            );
        }
        if ($useFreeAmount && $useBalance) {
            api_error(
                'El monto libre no se puede combinar con monto a favor.',
                'MONTO_LIBRE_SALDO_FAVOR',
                409
            );
        }
        if ($useFreeAmount && $receivedAmount !== null) {
            api_error(
                'El monto libre reemplaza al monto recibido. Usá una sola modalidad de importe.',
                'MONTO_LIBRE_MONTO_RECIBIDO',
                409
            );
        }
        ...
        $condoned = filter_var($body['condonado'] ?? false, FILTER_VALIDATE_BOOL);
        if ($condoned && ($useBalance || $useFreeAmount || ($receivedAmount !== null && $receivedAmount > 0))) {
            api_error(
                'Una condonación no puede utilizar monto a favor, monto recibido ni monto libre.',
                'IMPORTE_CONDONACION_INVALIDO',
                409
            );
        }
        $state = $condoned ? 'CONDONADO' : 'PAGADO';
        $paymentMethodId = $condoned
            ? null
            : self::normalPaymentMediumId($db, $body['id_medio_pago'] ?? null);
        $hasFees = $obligations !== [];
        ...
        $saved = transaction($db, static function () use (
            ...
            $charged = round(
                (float)($feesResult['monto'] ?? 0)
                + (float)($registrationResult['monto'] ?? 0),
                2
            );
            $referenceCharged = $charged;

            if ($state === 'PAGADO' && $useFreeAmount) {
                if ($freeAmount === null || $freeAmount <= 0) {
                    api_error('Indicá un monto libre válido.', 'MONTO_LIBRE_INVALIDO');
                }
                if ($referenceCharged <= 0 || $freeAmount + 0.005 >= $referenceCharged) {
                    api_error(
                        'El monto libre debe ser menor al importe normal del cobro. Para cobrar de más, usá monto recibido.',
                        'MONTO_LIBRE_FUERA_DE_RANGO',
                        409
                    );
                }
                ...
                self::aplicarMontoLibreOperacion($db, $customRows, $freeAmount);
                $charged = $freeAmount;
            }

            $balanceApplied = 0.0;
            $cashReceived = $charged;
            $surplus = 0.0;
            $availableBefore = 0.0;

            if ($state === 'PAGADO') {
                // El lock del socio serializa consumos/reversiones del mismo saldo.
                $availableBefore = self::saldoFavorActual($db, $principalId, true);
                if ($receivedAmount === null) {
                    $balanceApplied = $useBalance ? min($availableBefore, $charged) : 0.0;
                    $cashReceived = round($charged - $balanceApplied, 2);
                } else {
                    $cashReceived = $receivedAmount;
                    $missing = max(0.0, round($charged - $cashReceived, 2));
                    $balanceApplied = $useBalance ? min($availableBefore, $missing) : 0.0;
                    if ($cashReceived + $balanceApplied + 0.004 < $charged) {
                        api_error(
                ...
                $surplus = max(0.0, round($cashReceived + $balanceApplied - $charged, 2));
            }
            ...
            // Distribuir el monto a favor aplicado entre las líneas del cobro.
            $remainingBalance = $balanceApplied;
            $firstPaymentId = null;
            $firstRegistrationId = null;
            foreach ($rows as $row) {
                $lineAmount = max(0.0, (float)($row['monto'] ?? 0));
                $lineApplied = min($remainingBalance, $lineAmount);
                if ($row['tipo_registro'] === 'CUOTA') {
                    $firstPaymentId ??= (int)$row['id_linea'];
                    if ($lineApplied > 0) {
                        $update = $db->prepare(
                            'UPDATE pagos SET monto_saldo_favor_aplicado = ? WHERE id_pago = ?'
                        );
                        $update->execute([
                            number_format($lineApplied, 2, '.', ''),
                            (int)$row['id_linea'],
                        ]);
                    }
                } else {
                    $firstRegistrationId ??= (int)$row['id_linea'];
                    if ($lineApplied > 0) {
                        $update = $db->prepare(
                            'UPDATE pagos_inscripciones SET monto_saldo_favor_aplicado = ? WHERE id_pago_inscripcion = ?'
                        );
                        $update->execute([
                            number_format($lineApplied, 2, '.', ''),
                            (int)$row['id_linea'],
                ...
                $remainingBalance = round($remainingBalance - $lineApplied, 2);
            }
            if ($remainingBalance > 0.004) {
                api_error('No se pudo distribuir el monto a favor aplicado.', 'SALDO_FAVOR_DISTRIBUCION', 500);
            }

            $movementDate = valid_date($body['fecha_pago'] ?? date('Y-m-d'), 'pago') . ' ' . date('H:i:s');
            if ($balanceApplied > 0) {
                self::registrarMovimientoSaldo(
                    $db,
                    $principalId,
                    'APLICACION_PAGO',
                    -$balanceApplied,
                    ...
                    (int)$auth['id_usuario_master'],
                    $operationCode,
                    'SALDO-APLICACION-' . $operationCode,
                    'Monto a favor aplicado al cobro ' . $operationCode,
                    $movementDate
                );
            }
            if ($surplus > 0) {
                self::registrarMovimientoSaldo(
                    ...
                    (int)$auth['id_usuario_master'],
                    $operationCode,
                    'SALDO-SOBRANTE-' . $operationCode,
                    'Sobrante del cobro ' . $operationCode,
                    $movementDate
                );
            }

            // Volver a leer luego de distribuir el saldo para que comprobantes y
            // auditoría reflejen exactamente cuánto se cubrió con monto a favor.
            $rows = array_merge(
                self::paymentRows($db, null, $operationCode),
                self::registrationRows($db, null, $operationCode)
            );
            $operation = self::groupOperations($rows)[0] ?? null;
            $balanceAfter = $state === 'PAGADO'
                ? self::saldoFavorActual($db, $principalId)
                : $availableBefore;
            $concepts = [];
            ...
            audit_change(
                ...
                [
                    ...
                    'lineas' => $operation['lineas'] ?? [],
                    'monto_referencia' => number_format($referenceCharged, 2, '.', ''),
                    'monto_obligacion' => number_format($charged, 2, '.', ''),
                    'monto_libre' => $useFreeAmount ? number_format($charged, 2, '.', '') : null,
                    'monto_saldo_favor_aplicado' => number_format($balanceApplied, 2, '.', ''),
                    'monto_recibido' => number_format($cashReceived, 2, '.', ''),
                    'sobrante_saldo_favor' => number_format($surplus, 2, '.', ''),
                    'saldo_favor_resultante' => number_format($balanceAfter, 2, '.', ''),
                ]
            );
            ...
            return [
                ...
                'monto' => number_format($charged, 2, '.', ''),
                'monto_referencia' => number_format($referenceCharged, 2, '.', ''),
                'es_monto_libre' => $useFreeAmount,
                'monto_saldo_favor_aplicado' => number_format($balanceApplied, 2, '.', ''),
                'monto_cobrado_ahora' => number_format($cashReceived, 2, '.', ''),
                'sobrante_saldo_favor' => number_format($surplus, 2, '.', ''),
                'saldo_favor' => number_format($balanceAfter, 2, '.', ''),
                'incluye_cuotas' => $feesResult !== null,
                'incluye_inscripcion' => $registrationResult !== null,
    ...
    protected static function anularDatos(array $auth, array $body): array
    {
        ...
        $result = transaction($db, static function () use ($db, $auth, $code, $requested): array {
            ...
            // Si la operación movió monto a favor, la reversión pertenece al
            // cobro completo. Por seguridad se anulan todas sus líneas activas,
            // aunque una petición manual haya enviado solo una de ellas.
            $hasBalanceMovement = $db->prepare(
                "SELECT 1 FROM saldos_favor_movimientos
                 WHERE referencia_externa = ?
                   AND tipo IN ('SOBRANTE','APLICACION_PAGO')
                 LIMIT 1"
            );
            $hasBalanceMovement->execute([$code]);
            if ($hasBalanceMovement->fetchColumn()) {
                $allPayments = $db->prepare(
                    "SELECT id_pago FROM pagos
                     WHERE codigo_operacion = ? AND estado IN ('PAGADO','CONDONADO')
                     ORDER BY id_pago FOR UPDATE"
                );
                $allPayments->execute([$code]);
                foreach ($allPayments->fetchAll() as $row) {
                    $paymentId = (int)$row['id_pago'];
                    $expanded['CUOTA-' . $paymentId] = [
                        'tipo' => 'CUOTA',
                        'id_linea' => $paymentId,
                ...
                $allRegistrations = $db->prepare(
                    "SELECT id_pago_inscripcion FROM pagos_inscripciones
                     WHERE codigo_operacion = ? AND estado IN ('PAGADO','CONDONADO')
                     ORDER BY id_pago_inscripcion FOR UPDATE"
                );
                $allRegistrations->execute([$code]);
                foreach ($allRegistrations->fetchAll() as $row) {
                    $registrationId = (int)$row['id_pago_inscripcion'];
                    $expanded['INSCRIPCION-' . $registrationId] = [
                        'tipo' => 'INSCRIPCION',
                        'id_linea' => $registrationId,
            ...
            if (count($expanded) > 1000) {
                api_error('La operación contiene demasiadas líneas para anular.', 'VALIDATION_ERROR');
            }
            ksort($expanded);
            ...
            // Revertir el libro de montos a favor de la operación antes de
            // eliminar las líneas. Si el sobrante ya fue utilizado por el socio,
            // la anulación se bloquea para no producir un saldo negativo.
            $balanceMovements = $db->prepare(
                "SELECT * FROM saldos_favor_movimientos
                 WHERE referencia_externa = ?
                   AND tipo IN ('SOBRANTE','APLICACION_PAGO')
                 ORDER BY id_movimiento
                 FOR UPDATE"
            );
            $balanceMovements->execute([$code]);
            $movementsToReverse = $balanceMovements->fetchAll();
            $reversalByPartner = [];
            foreach ($movementsToReverse as $movement) {
                $partnerId = (int)$movement['id_socio'];
                $reversalByPartner[$partnerId] = ($reversalByPartner[$partnerId] ?? 0.0)
                    - (float)$movement['monto'];
            }
            foreach ($reversalByPartner as $partnerId => $delta) {
                $currentBalance = self::saldoFavorActual($db, (int)$partnerId, true);
                if ($currentBalance + $delta < -0.004) {
                    api_error(
                        'No se puede eliminar el cobro porque el sobrante generado por esa operación ya fue utilizado.',
                        'SALDO_FAVOR_YA_UTILIZADO',
                        409
                    );
                }
            }
            $reversedMovements = 0;
            foreach ($movementsToReverse as $movement) {
                $originalAmount = (float)$movement['monto'];
                if (abs($originalAmount) < 0.005) continue;
                self::registrarMovimientoSaldo(
                    $db,
                    (int)$movement['id_socio'],
                    'REVERSO',
                    -$originalAmount,
                    'SISTEMA',
                    $movement['id_pago'] === null ? null : (int)$movement['id_pago'],
                    $movement['id_pago_inscripcion'] === null ? null : (int)$movement['id_pago_inscripcion'],
                    $movement['id_medio_pago'] === null ? null : (int)$movement['id_medio_pago'],
                    (int)$auth['id_usuario_master'],
                    $code,
                    'SALDO-REVERSO-' . (int)$movement['id_movimiento'],
                    'Reverso por eliminación del cobro ' . $code
                );
                $reversedMovements++;
            }
            ...
            try {
                audit_change(
                    ...
                    [
                        ...
                        'movimientos_saldo_revertidos' => $reversedMovements,
                    ]
                );
