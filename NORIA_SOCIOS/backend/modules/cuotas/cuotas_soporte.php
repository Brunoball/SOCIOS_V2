...
abstract class CuotasSoporte
{
    ...
    protected static function saldoFavorActual(PDO $db, int $partnerId, bool $lockPartner = false): float
    {
        if ($lockPartner) {
            $lock = $db->prepare('SELECT s.id_socio FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio) FOR UPDATE');
            $lock->execute([$partnerId]);
            if (!$lock->fetchColumn()) {
                api_error('El socio no existe.', 'SOCIO_NO_DISPONIBLE', 404);
            }
        }
    ...
    protected static function registrarMovimientoSaldo(
        PDO $db,
        int $partnerId,
        string $type,
        float $amount,
        ...
        $allowedTypes = ['SOBRANTE', 'APLICACION_PAGO', 'REVERSO', 'AJUSTE_CREDITO', 'AJUSTE_DEBITO'];
        if (!in_array($type, $allowedTypes, true)) {
            throw new InvalidArgumentException('Tipo de movimiento de saldo no permitido.');
        }
        if (!in_array($origin, ['BOT', 'SISTEMA', 'MANUAL'], true)) {
            throw new InvalidArgumentException('Origen de movimiento de saldo no permitido.');
        }
        if (abs($amount) < 0.005) return 0;

        $statement = $db->prepare(
            'INSERT INTO saldos_favor_movimientos
             (id_socio, fecha, tipo, monto, origen, id_pago, id_pago_inscripcion, id_medio_pago,
              id_usuario_master, referencia_externa, clave_idempotencia, detalle)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        $statement->execute([
            $partnerId,
    ...
    {
        $statement = $db->prepare('SELECT s.id_socio, s.activo FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)');
        ...
        $statement = $db->prepare(
            'SELECT sc.id_socio_categoria
             ...
             WHERE sc.id_socio = ? AND sc.id_categoria = ?
               AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = sc.id_socio)
               AND sc.fecha_desde <= ?
    ...
    protected static function discountContextForPartner(PDO $db, int $partnerId, array $rules, array &$cache): array
    {
        ...
        $statement = $db->prepare(
            'SELECT f.id_familia,
                    COUNT(CASE WHEN sm.activo = 1 AND NOT EXISTS (SELECT 1 FROM socios_eliminados sem WHERE sem.id_socio = sm.id_socio) THEN 1 END) AS cantidad_integrantes
             FROM socios s
             ...
             LEFT JOIN familia_socios fsm ON fsm.id_familia = f.id_familia AND fsm.fecha_desvinculacion IS NULL
    ...
    {
        $statement = $db->prepare('SELECT CONCAT(s.apellido, ", ", s.nombre) AS socio, s.dni FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)');
        $statement->execute([$partnerId]);
        $row = $statement->fetch();
    ...
    {
        $rows = $db->query(
            'SELECT f.id_familia, COUNT(CASE WHEN s.activo = 1 AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio) THEN 1 END) AS cantidad
             FROM familias f
             LEFT JOIN familia_socios fs ON fs.id_familia = f.id_familia AND fs.fecha_desvinculacion IS NULL
