...
function configuracion_cantidad_usos(PDO $db, array $definition, int $id): int
{
    switch ($definition['lista']) {
        case 'medios_pago':
            $statement = $db->prepare(
                'SELECT
                    ...
                    + (SELECT COUNT(*) FROM ventas_operaciones WHERE id_medio_pago = ?)'
            );
            $statement->execute([$id, $id, $id, $id, $id]);
        ...
        case 'contable_proveedores':
            $statement = $db->prepare(
                'SELECT
                    ...
                    + (SELECT COUNT(*) FROM ventas_configuraciones WHERE id_proveedor_contable = ?)'
            );
        ...
        case 'contable_categorias_ingreso':
            $statement = $db->prepare(
                'SELECT
                    (SELECT COUNT(*) FROM contable_ingresos WHERE id_categoria = ?)
                    + (SELECT COUNT(*) FROM ventas_configuraciones WHERE id_categoria_contable = ?)'
            );
        ...
        case 'contable_conceptos_ingreso':
            $statement = $db->prepare(
                'SELECT
                    (SELECT COUNT(*) FROM contable_ingresos WHERE id_concepto = ?)
                    + (SELECT COUNT(*) FROM ventas_configuraciones WHERE id_concepto_contable = ?)'
            );
            $statement->execute([$id, $id]);
            return (int)$statement->fetchColumn();
