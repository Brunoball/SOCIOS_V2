...
final class Socios
{
    ...
    public static function eliminarDefinitivamente(): never
    {
        $auth = require_admin();
        $body = request_body();
        $id = positive_id($body['id'] ?? null, 'socio');
        $reason = required_text($body, 'motivo', 'motivo de eliminación', 500);
        api_success(
            self::eliminarDefinitivamenteDatos($auth, $id, $reason),
            'Socio eliminado de la operatoria. Su historial contable quedó conservado.'
        );
    }
