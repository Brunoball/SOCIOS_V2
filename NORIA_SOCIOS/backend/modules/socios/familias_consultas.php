    ...
    private static function listarDatos(PDO $db, array $filters): array
    {
        ...
        if ($search !== '') {
            $where[] = '(f.nombre LIKE :buscar_familia OR f.descripcion LIKE :buscar_descripcion OR EXISTS (
                ...
                WHERE fss.id_familia = f.id_familia AND fss.fecha_desvinculacion IS NULL
                  AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = ss.id_socio)
                  AND (ss.nombre LIKE :buscar_socio_nombre OR ss.apellido LIKE :buscar_socio_apellido OR ss.dni LIKE :buscar_socio_dni)
            ))';
            $term = '%' . $search . '%';
        ...
        $members = $db->prepare(
            "SELECT fs.id_familia, s.id_socio, s.nombre, s.apellido, s.dni, s.activo
             ...
             WHERE fs.id_familia IN ({$placeholders})
               AND fs.fecha_desvinculacion IS NULL
               AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)
             ORDER BY fs.id_familia, s.apellido, s.nombre"
        );
        $members->execute($ids);
