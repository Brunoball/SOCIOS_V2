    ...
    private static function guardarDatos(array $auth, array $body): array
    {
        ...
        try {
            $saved = transaction($db, static function () use ($db, $auth, $id, $name, $description, $memberIds): array {
                    ...
                    "SELECT s.id_socio
                     ...
                     WHERE s.id_socio IN ({$placeholders}) AND (
                        s.activo = 1 OR EXISTS (
                            SELECT 1 FROM familia_socios fs WHERE fs.id_socio = s.id_socio AND fs.id_familia = ? AND fs.fecha_desvinculacion IS NULL
                        )
                     )
                     FOR UPDATE"
                );
                ...
                // pero el vínculo histórico no se borra: se cierra.
                $releaseInactive = $db->prepare(
                    "UPDATE familia_socios fs
                     INNER JOIN familias f ON f.id_familia = fs.id_familia
                     SET fs.fecha_desvinculacion = CURDATE(),
                         fs.motivo_desvinculacion = 'REASIGNADO DESDE FAMILIA INACTIVA',
                         fs.socio_vinculo_activo = NULL
                     WHERE fs.id_socio IN ({$placeholders})
                       AND fs.id_familia <> ?
                       AND fs.fecha_desvinculacion IS NULL
                       AND f.activo = 0"
                ...
                // Sincronización histórica: cerrar integrantes retirados y crear
                // solamente vínculos nuevos. Los vínculos anteriores quedan para
                // auditoría y trazabilidad.
                $currentLinks = $db->prepare(
                    'SELECT id_familia_socio, id_socio
                     FROM familia_socios
                     WHERE id_familia = ? AND fecha_desvinculacion IS NULL
                     FOR UPDATE'
                );
                $currentLinks->execute([$familyId]);
                $currentByPartner = [];
                foreach ($currentLinks->fetchAll() as $link) {
                    $currentByPartner[(int)$link['id_socio']] = (int)$link['id_familia_socio'];
                }
                $selected = array_fill_keys($memberIds, true);
                $closeLink = $db->prepare(
                    "UPDATE familia_socios
                     SET fecha_desvinculacion = CURDATE(),
                         motivo_desvinculacion = 'RETIRADO DE LA FAMILIA',
                         socio_vinculo_activo = NULL
                     WHERE id_familia_socio = ?"
                );
                foreach ($currentByPartner as $memberId => $linkId) {
                    if (!isset($selected[$memberId])) $closeLink->execute([$linkId]);
                }

                $insert = $db->prepare(
                    'INSERT INTO familia_socios (id_familia, id_socio, socio_vinculo_activo)
                     VALUES (?, ?, ?)'
                );
                foreach ($memberIds as $memberId) {
                    if (!isset($currentByPartner[$memberId])) {
                        $insert->execute([$familyId, $memberId, $memberId]);
                    }
                }
