...
final class Dashboard
{
    ...
    private static function resumenDatos(PDO $db): array
    {
        ...
        $activePartners = self::count($db, 'SELECT COUNT(*) FROM socios s WHERE s.activo = 1 AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)');
        $inactivePartners = self::count($db, 'SELECT COUNT(*) FROM socios s WHERE s.activo = 0 AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)');
        $newPartners = self::count(
            $db,
            'SELECT COUNT(*) FROM socios s WHERE s.fecha_ingreso >= ? AND s.fecha_ingreso < ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio)',
        ...
        $partnerIncome = self::sumCents(
            $db,
            "SELECT COALESCE(SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)), 0) FROM pagos
             WHERE estado = 'PAGADO' AND fecha_pago >= ? AND fecha_pago < ?",
            [$monthStart->format('Y-m-d'), $monthEnd->format('Y-m-d')]
        ) + self::sumCents(
            $db,
            "SELECT COALESCE(SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)), 0) FROM pagos_inscripciones
             WHERE estado = 'PAGADO' AND fecha_pago >= ? AND fecha_pago < ?",
            [$monthStart->format('Y-m-d'), $monthEnd->format('Y-m-d')]
        ) + self::sumCents(
            $db,
            "SELECT COALESCE(SUM(monto), 0) FROM saldos_favor_movimientos
    ...
    private static function monthlySeries(PDO $db, DateTimeImmutable $start, DateTimeImmutable $end): array
    {
        $statement = $db->prepare(
            "SELECT DATE_FORMAT(fecha, '%Y-%m') AS periodo,
             ...
             FROM (
                 SELECT fecha_pago AS fecha, (monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS ingresos_socios, 0 AS otros_ingresos, 0 AS egresos
                 FROM pagos WHERE estado = 'PAGADO'
                 UNION ALL
                 SELECT fecha_pago AS fecha, (monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS ingresos_socios, 0 AS otros_ingresos, 0 AS egresos
                 FROM pagos_inscripciones WHERE estado = 'PAGADO'
                 UNION ALL
                 SELECT fecha, monto AS ingresos_socios, 0 AS otros_ingresos, 0 AS egresos
                 FROM saldos_favor_movimientos WHERE tipo = 'SOBRANTE' AND monto > 0
                 UNION ALL
             ...
             FROM (
                 SELECT 'INGRESO_SOCIOS' AS tipo,
                        ...
                        CASE
                            ...
                            WHEN SUM(cobros.es_inscripcion) > 0
                                THEN 'INSCRIPCIONES'
                            ELSE 'SALDO A FAVOR'
                        END AS titulo,
                 ...
                 FROM (
                     SELECT
                         ...
                         (p.monto - COALESCE(p.monto_saldo_favor_aplicado, 0)) AS importe,
                         1 AS es_cuota,
                         0 AS es_inscripcion,
                         0 AS es_sobrante
                     FROM pagos p
                     ...
                     SELECT
                         ...
                         (pi.monto - COALESCE(pi.monto_saldo_favor_aplicado, 0)) AS importe,
                         0 AS es_cuota,
                         1 AS es_inscripcion,
                         0 AS es_sobrante
                     ...
                     SELECT
                         COALESCE(NULLIF(sf.referencia_externa, ''), CONCAT('SALDO-', sf.id_movimiento)) AS operacion,
                         DATE(sf.fecha) AS fecha,
                         sf.created_at AS creado,
                         sf.monto AS importe,
                         0 AS es_cuota,
                         0 AS es_inscripcion,
                         1 AS es_sobrante
                     FROM saldos_favor_movimientos sf
                     WHERE sf.tipo = 'SOBRANTE' AND sf.monto > 0
                 ) cobros
