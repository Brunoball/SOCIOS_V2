...
trait VentasSoporte
{
    ...
    protected static function socioVenta(PDO $db, int $id): array
    {
        $statement = $db->prepare(
            'SELECT id_socio, nombre, apellido, dni, telefono, email, activo
             FROM socios s WHERE s.id_socio = ? AND NOT EXISTS (SELECT 1 FROM socios_eliminados se WHERE se.id_socio = s.id_socio) LIMIT 1'
        );
        $statement->execute([$id]);
        $row = $statement->fetch();
    ...
    protected static function normalizarItems(
        PDO $db,
        mixed $rawItems,
        bool $allowsManualPrice,
        array $previousItems = []
    ): array {
        ...
        foreach ($previousItems as $previous) {
            $previousId = (int)($previous['id_item'] ?? 0);
            if ($previousId > 0) $previousById[$previousId] = $previous;
        }
        ...
        foreach ($rawItems as $raw) {
            ...
            $itemId = self::idOpcionalVenta($raw['id_item'] ?? null, 'ítem de venta');
            $previous = null;

            if ($itemId !== null) {
                if (isset($seenHistoricalIds[$itemId])) {
                    api_error('Una línea histórica de la venta está repetida.', 'ITEM_VENTA_DUPLICADO', 409);
                }
                $seenHistoricalIds[$itemId] = true;
                $previous = $previousById[$itemId] ?? null;
                if (!$previous || (int)$previous['id_producto'] !== $productId) {
                    api_error(
                        'Una línea histórica de la venta ya no coincide con el producto original.',
                        'ITEM_VENTA_INVALIDO',
                        409
                    );
                }
            ...
            // Las líneas históricas no se mezclan con líneas nuevas: conservan su snapshot.
            $key = $itemId !== null
                ? 'HISTORICO|' . $itemId
                : 'NUEVO|' . $productId . '|' . ($allowsManualPrice ? trim((string)$priceInput) : 'CATALOGO');
        ...
        foreach ($merged as $raw) {
            ...
            $isHistorical = is_array($raw['previous']);
            if (!$product) {
                api_error('Uno de los productos de la venta ya no existe.', 'PRODUCTO_NO_ENCONTRADO', 409);
            }
            if (!$isHistorical && !(bool)$product['activo']) {
            ...
            if ($isHistorical) {
                ...
                $price = $allowsManualPrice
                    ...
                    : number_format((float)$previous['precio_unitario'], 2, '.', '');
                $codeSnapshot = $previous['producto_codigo_snapshot'];
                $nameSnapshot = (string)$previous['producto_nombre_snapshot'];
                $controlsStock = (bool)$previous['controla_stock_snapshot'];
            } else {
                $price = $allowsManualPrice
                    ? decimal_amount($raw['precio_input'], 'precio unitario', 0, 999999999999.99)
                    : number_format((float)$product['precio'], 2, '.', '');
                $codeSnapshot = $product['codigo'];
                $nameSnapshot = (string)$product['nombre'];
                $controlsStock = (bool)$product['controla_stock'];
            }
            ...
            $items[] = [
                ...
                'producto_codigo_snapshot' => $codeSnapshot,
                'producto_nombre_snapshot' => $nameSnapshot,
                'cantidad' => $raw['cantidad'],
                'precio_unitario' => $price,
                'subtotal' => number_format($lineCents / 100, 2, '.', ''),
                'controla_stock' => $controlsStock,
            ];
        }
        if ($subtotalCents <= 0) {
