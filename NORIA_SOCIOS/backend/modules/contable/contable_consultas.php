...
trait ContableConsultas
{
    ...
    protected static function resumenDatos(PDO $db, int $year, int $selectedMonth): array
    {
        ...
        self::acumularTotalesMensuales(
            $db,
            "SELECT MONTH(fecha_pago) AS mes, SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS total
             FROM pagos
        ...
        self::acumularTotalesMensuales(
            ...
            "SELECT MONTH(fecha) AS mes, SUM(monto) AS total
             ...
             WHERE tipo = 'SOBRANTE' AND monto > 0 AND fecha >= ? AND fecha < ?
    ...
    private static function resumenCategoriasIngresos(PDO $db, int $year, int $month): array
    {
        ...
        self::acumularAgrupacion(
            $db,
            "SELECT COALESCE(NULLIF(categoria_nombre_snapshot, ''), 'CUOTAS DE SOCIOS') AS nombre, SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS total
             FROM pagos
             WHERE estado = 'PAGADO' AND fecha_pago >= ? AND fecha_pago < ?
        ...
        self::acumularAgrupacion(
            $db,
            "SELECT COALESCE(NULLIF(categoria_nombre_snapshot, ''), 'INSCRIPCIONES') AS nombre, SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS total
             FROM pagos_inscripciones
             WHERE estado = 'PAGADO' AND fecha_pago >= ? AND fecha_pago < ?
        ...
        self::acumularAgrupacion(
            $db,
            "SELECT 'SALDO A FAVOR' AS nombre, SUM(monto) AS total
             FROM saldos_favor_movimientos
             WHERE tipo = 'SOBRANTE' AND monto > 0 AND fecha >= ? AND fecha < ?
             HAVING SUM(monto) > 0",
            [$monthStart, $monthEnd],
    ...
    private static function resumenMedios(PDO $db, int $year, int $month): array
    {
        ...
        self::acumularAgrupacion(
            $db,
            "SELECT COALESCE(NULLIF(medio_pago_nombre_snapshot, ''), 'SIN MEDIO') AS nombre, SUM(monto - COALESCE(monto_saldo_favor_aplicado, 0)) AS total
             FROM pagos
             WHERE estado = 'PAGADO' AND fecha_pago >= ? AND fecha_pago < ?
        ...
        self::acumularAgrupacion(
            $db,
            "SELECT COALESCE(mp.nombre, 'SIN MEDIO') AS nombre, SUM(sf.monto) AS total
             FROM saldos_favor_movimientos sf
             LEFT JOIN medios_pago mp ON mp.id_medio_pago = sf.id_medio_pago
             WHERE sf.tipo = 'SOBRANTE' AND sf.monto > 0 AND sf.fecha >= ? AND sf.fecha < ?
             GROUP BY COALESCE(mp.nombre, 'SIN MEDIO')",
            [$monthStart, $monthEnd],
            $totals
        );
        self::acumularAgrupacion(
    ...
    /**
     * Describe por qué el importe efectivamente ingresado puede diferir del
     * valor de referencia de la cuota. Se mantiene estructurado para que el
     * frontend pueda mostrar los mismos chips que el sistema RH sin mezclar
     * presentación con la consulta contable.
     */
    private static function ajustesMontoIngresoSocio(array $item): array
    {
        if (($item['origen'] ?? '') === 'SALDO_FAVOR') {
            return [[
                'tipo' => 'SALDO_FAVOR_SOBRANTE',
                'monto' => self::importeDesdeCentavos(self::centavos($item['monto'] ?? 0)),
            ]];
        }

        $adjustments = [];
        $familyDiscount = max(0.0, min(100.0, (float)($item['porcentaje_descuento_familiar'] ?? 0)));
        $modalityDiscount = max(0.0, min(100.0, (float)($item['porcentaje_descuento_modalidad'] ?? 0)));
        $baseAmount = (float)($item['monto_base'] ?? 0);
        $obligationAmount = (float)($item['monto_obligacion'] ?? 0);
        $balanceApplied = max(0.0, (float)($item['monto_saldo_favor_aplicado'] ?? 0));

        if ($familyDiscount > 0.004) {
            $adjustments[] = [
                'tipo' => 'DESCUENTO_FAMILIAR',
                'porcentaje' => number_format($familyDiscount, 2, '.', ''),
            ];
        }

        // También cubre registros históricos/manuales: si el importe de la
        // obligación no coincide con el cálculo automático guardado, se marca
        // como monto personalizado. Los cobros normales actuales no disparan
        // esta etiqueta porque conservan exactamente ese cálculo.
        if ($baseAmount > 0.004) {
            $expectedAmount = round(
                $baseAmount
                * (1 - ($modalityDiscount / 100))
                * (1 - ($familyDiscount / 100)),
                2
            );
            if (abs($obligationAmount - $expectedAmount) > 0.009) {
                $adjustments[] = ['tipo' => 'MONTO_PERSONALIZADO'];
            }
        }

        if ($balanceApplied > 0.004) {
            $adjustments[] = [
                'tipo' => 'SALDO_FAVOR_APLICADO',
                'monto' => self::importeDesdeCentavos(self::centavos($balanceApplied)),
            ];
        }
    ...
    protected static function listarIngresosSociosDatos(PDO $db, array $filters): array
    {
        ...
        $items = array_merge(
            ...
            self::listarSobrantesSaldoFavor($db, $monthStart, $monthEnd, $categoryId, $paymentMethodId)
        );
        ...
        foreach ($items as &$item) {
            ...
            $item['ajustes_monto'] = self::ajustesMontoIngresoSocio($item);
    ...
    private static function listarCuotasCobradas(
        ...
        $statement = $db->prepare(
            "SELECT CONCAT('CUOTA-', p.id_pago) AS clave,
                    ...
                    p.id_categoria,
                    p.id_medio_pago,
                    COALESCE(NULLIF(p.socio_nombre_snapshot, ''), NULLIF(se.socio_nombre, ''), CONCAT(COALESCE(s.apellido, ''), ', ', COALESCE(s.nombre, ''))) AS socio,
                    COALESCE(NULLIF(p.socio_dni_snapshot, ''), NULLIF(se.documento, ''), s.dni, '') AS dni,
                    ...
                    COALESCE(p.monto_base, p.monto) AS monto_base,
                    COALESCE(p.porcentaje_descuento_modalidad, 0) AS porcentaje_descuento_modalidad,
                    COALESCE(p.porcentaje_descuento_familiar, 0) AS porcentaje_descuento_familiar,
                    p.monto AS monto_obligacion,
                    (p.monto - COALESCE(p.monto_saldo_favor_aplicado, 0)) AS monto,
                    COALESCE(p.monto_saldo_favor_aplicado, 0) AS monto_saldo_favor_aplicado,
                    NULL AS detalle
             FROM pagos p
    ...
    private static function listarInscripcionesCobradas(
        ...
        $statement = $db->prepare(
            "SELECT CONCAT('INSCRIPCION-', pi.id_pago_inscripcion) AS clave,
                    ...
                    pi.id_categoria,
                    pi.id_medio_pago,
                    COALESCE(NULLIF(pi.socio_nombre_snapshot, ''), NULLIF(se.socio_nombre, ''), CONCAT(COALESCE(s.apellido, ''), ', ', COALESCE(s.nombre, ''))) AS socio,
                    COALESCE(NULLIF(pi.socio_dni_snapshot, ''), NULLIF(se.documento, ''), s.dni, '') AS dni,
                    ...
                    pi.anio,
                    NULL AS mes_pagado,
                    COALESCE(pi.monto_base, pi.monto) AS monto_base,
                    0 AS porcentaje_descuento_modalidad,
                    COALESCE(pi.porcentaje_descuento_familiar, 0) AS porcentaje_descuento_familiar,
                    pi.monto AS monto_obligacion,
                    (pi.monto - COALESCE(pi.monto_saldo_favor_aplicado, 0)) AS monto,
                    COALESCE(pi.monto_saldo_favor_aplicado, 0) AS monto_saldo_favor_aplicado,
                    NULL AS detalle
             FROM pagos_inscripciones pi
             LEFT JOIN socios s ON s.id_socio = pi.id_socio
             LEFT JOIN socios_eliminados se ON se.id_socio = pi.id_socio
    ...
    private static function listarSobrantesSaldoFavor(
        PDO $db,
        string $start,
        string $end,
        ?int $categoryId,
        ...
        // Un sobrante no pertenece a una categoría particular; cuando el usuario
        // filtra por categoría se oculta para no atribuirlo incorrectamente.
        if ($categoryId !== null) return [];

        $where = ["sf.tipo = 'SOBRANTE'", 'sf.monto > 0', 'sf.fecha >= ?', 'sf.fecha < ?'];
        $params = [$start, $end];
        if ($paymentMethodId !== null) {
            $where[] = 'sf.id_medio_pago = ?';
            $params[] = $paymentMethodId;
        }

        $statement = $db->prepare(
            "SELECT CONCAT('SALDO-', sf.id_movimiento) AS clave,
                    'SALDO_FAVOR' AS origen,
                    sf.id_movimiento AS id_registro,
                    sf.referencia_externa AS codigo_operacion,
                    DATE(sf.fecha) AS fecha,
                    sf.created_at,
                    sf.id_socio,
                    0 AS id_categoria,
                    COALESCE(sf.id_medio_pago, 0) AS id_medio_pago,
                    COALESCE(NULLIF(se.socio_nombre, ''), CONCAT(COALESCE(s.apellido, ''), ', ', COALESCE(s.nombre, ''))) AS socio,
                    COALESCE(NULLIF(se.documento, ''), s.dni, '') AS dni,
                    'SALDO A FAVOR' AS categoria,
                    COALESCE(mp.nombre, 'SIN MEDIO') AS medio,
                    'SOBRANTE' AS modalidad_codigo,
                    'SOBRANTE A SALDO A FAVOR' AS modalidad,
                    'SOBRANTE A SALDO A FAVOR' AS periodo,
                    YEAR(sf.fecha) AS anio,
                    NULL AS mes_pagado,
                    sf.monto AS monto_base,
                    0 AS porcentaje_descuento_modalidad,
                    0 AS porcentaje_descuento_familiar,
                    sf.monto AS monto_obligacion,
                    sf.monto AS monto,
                    0 AS monto_saldo_favor_aplicado,
                    COALESCE(NULLIF(sf.detalle, ''), 'Sobrante guardado como monto a favor') AS detalle
             FROM saldos_favor_movimientos sf
             LEFT JOIN socios s ON s.id_socio = sf.id_socio
             LEFT JOIN socios_eliminados se ON se.id_socio = sf.id_socio
             LEFT JOIN medios_pago mp ON mp.id_medio_pago = sf.id_medio_pago
             WHERE " . implode(' AND ', $where)
        );
        $statement->execute($params);
